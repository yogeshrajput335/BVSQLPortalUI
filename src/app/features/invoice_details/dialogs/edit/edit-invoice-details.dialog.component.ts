...
export class EditInvoiceDetailsDialogComponent {
  ...
  terms :any;
  employees:any;
  product:any={employee:'',employeeId:0,perHourCost:0,totalHours:0,totalCost:0,projectId:0,invoiceId:0};
  products:any=[];
  projects:any=[];
  empPerHours:any=[];
  clientTerms=[];
  ...
  confirmEdit(): void {
    this.dataService.updateInvoiceDetails(this.data,this.products);
  }
  public AddProduct(){
    if(this.isEmpHavingPerHour){
      this.products.push(this.product);
      this.product = {employee:'',employeeId:0,perHourCost:0,totalHours:0,totalCost:0,projectId:0,invoiceId:0};
    } else {
  ...
  public onEmployeeChange(){
    ...
    this.product.employee = e.firstName+ ' '+ e.lastName;
